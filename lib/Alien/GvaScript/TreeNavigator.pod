...
the arrow keys or the C<TAB>
key. To see the whole page at once (for example for
printing), press the C<S<Ctrl-keypad *>> key.
...
When C<< tabIndex >= 0 >>, the keymap created for
capturing keyboard events is bound to the 
tree element; otherwise it is bound globally to the 
C<document> element. So another reason for 
using tabbing is when there are several trees on the 
same page : otherwise a single keypress would 
navigate in all trees simultaneously!
...
or close the node, but will also select that node; default is false.
...
Makes sure that the selected is visible in the central area of 
...
If true (the default), the first node is selected just after constructing
the tree navigator. 
