  ...
  <script>
    var gvascript_form = new GvaScript.Form('my_form', {
      ...
      onSubmit: function(gva_form) {
        ...
        // do form submission
        // ....
        if(success)               {gva_form.notify('onAfterSubmit', arg)}
        else 
        if(data_validation_error) {gva_form.notify('onDataValidationError', arg)}
        else                      {gva_form.notify('onSubmitFailure', arg)} 
      },
...
Signature
  ...
  function onSubmit(gva_form) {
    // gva_form : GvaScript.Form instance
    // use to notify 'onAfterSubmit', 'onSubmitFailure' or 'onDataValidationError'
  }

=item onAfterSubmit, onSubmitFailure, onDataValidationError

functions that correspond to different events that might occur after the onSubmit of the form.

the onSubmit handler must notify the form instance on which event to fire next.
...
=head2 notify
  ...
  gvascript_form.notify(eventName[, arg])

notifies the gvascript_form instance with an event to fire the corresponding handler(s).

C<eventName>: eventName to fire.

Supported C<eventName>s: B<onInit, onAfterSubmit, onSubmitFailure, onDataValidationError>

C<arg> argument to send over to the corresponding handler(s).

this method will first execute the general observers set to the eventName.
will stop if any of these observers returns C<false>.
if not, will continue to execute the instance specific handler.
...
namespace that holds all active instances of GvaScript.Form plus general observers to be applied on all instances.

instances are unique per form id that the instance is wrapped around.
...
=head3 addObserver
  ...
  GvaScript.Forms.addObserver({eventName: handler}[, {eventName: handler}[, ...]]})

adding one or more general form observers. to be applied on all instances of GvaScript.Form and will be 
executed I<before> the instance specific handler.

supported eventNames C<onInit, onBeforeSubmit, onAfterSubmit, onSubmitFailure, onDataValidationError>.
