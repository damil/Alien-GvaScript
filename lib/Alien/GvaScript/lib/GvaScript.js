...
//        }
  ...
  firstSubNode: function (node) {
    ...
    var nextNode = Element.navigateDom(parent.firstChild, 'nextSibling',
                                            this.classes.nodeOrLeaf);
    if ( !nextNode && parent && !this.isLeaf(node) ) {
      var nextNodes = Element.select(parent, 'div:([class~='+this.classes.node+']');
...
/*  firstSubNode: function (node) {
  ...
  */
  ...
  nextDisplayedNode: function (node) {
    ...
    // that is of type TN_node or TN_leaf
    if (!this.isClosed(node)) {
...
/*
    node = orig_node;
    // case 3: there's no direct sibling but another nodes
    // exist that are encapsulated in other div elements
    while ( node ) {
      Element.writeAttribute(node, 'visited', 'true');
      var parent_div = Element.up(node, "div");
      var nextNodes = Element.select(parent_div, 'div:([class~='+this.classes.node+'])');
      alert(nextNodes.length);
      if (nextNodes.length > 1) {
        return nextNodes[ptr];
      } 
       
      for (var i=0; i<nextNodes.length; i++) {
        var elem = nextNodes[i];
        if (elem.readAttribute("visited")) {
            continue;  
        } else {
            return elem;            
        }
...
//----------------------------------------------------------------------
  ...
  htmlForChoices: function(){ // creates the innerHTML 
    var isTable = this.options.showAsTable;
    var html = isTable ? "<table>" : "";
    for (var i = 0; i < this.choices.length; i++) {
      ...
      html += isTable ? this.choiceElementHTMLTable(label, id) : this.choiceElementHTML(label, id);
    }
    html += isTable ? "</table>" : "";
    return html;
  },
  ...
  choiceElementHTMLTable: function(label, id) {
    return "<tr class='" + this.classes.choiceItem +  "' id='" + id + "'>" + label + "</tr>";
  },
