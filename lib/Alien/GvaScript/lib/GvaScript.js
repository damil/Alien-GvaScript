...
Object.extend(GvaScript.Form, function() {
  // private methods
  function _fill_from_array(form, field_prefix, array, is_init) {
     for (var i=0; i < array.length; i++) {
       ...
       // otherwise try to walk down to a repetition block
         ...
         if (elem)
         GvaScript.Form.fill_from_tree(form, new_prefix, array[i]);
       }           
     }
  }

  function _fill_from_value(elem, val, is_init) {
      elem = $(elem);
...
console.log(this)
    ...
    fill_from_tree : function(form, field_prefix, tree) {
        ...
        case "string":
            var elem = form[new_prefix];
            if (elem) 
            _fill_from_value(elem, val, this.is_init); 
            break;

        case "object":
            if (val instanceof Array) 
            _fill_from_array(form, new_prefix, val, this.is_init);
            else 
...
console.log('Form.Prototype %o', this)
            var defaults = {
