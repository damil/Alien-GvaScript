...
//----------------------------------------------------------------------
  ...
  _mkDropdownDiv : function() {
    ...
    // positioning
    var coords  = Element.viewportOffset(this.inputElement);
    var offsets = Element.cumulativeScrollOffset(Element.getOffsetParent(this.inputElement));
    var dim    = Element.getDimensions(this.inputElement);
    
    div.style.left      = coords[0] + offsets[0] + this.options.offsetX - movedLeftBy + "px";
    div.style.top       = coords[1] + offsets[1] + dim.height -movedUpBy + "px";
