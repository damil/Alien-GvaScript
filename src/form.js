  ...
  fill_from_tree : (function() {
    ...
    var _fill_from_value = function(elem, val, is_init) {
      ...
      switch (elem_type) {
        ...
        case "checkbox" :
        case "radio":
          old_value = Form.Element.getValue(elem);
      ...
      try {
      if(is_init)
      Element.fire(elem, 'value:init',   {newvalue: elem.getValue()});
      else
      Element.fire(elem, 'value:change', {oldvalue: old_value, newvalue: elem.getValue()});
      } catch(e) {}
    }
    ...
    var _fill_from_array = function (form, field_prefix, array, is_init) {
      for (var i=0; i < array.length; i++) {
        ...
        elem = document.getElementById(new_prefix);  // TODO : check: is elem in form ?

        // no repetition block found, try to instanciate one
        if (!elem) {
          var placeholder = document.getElementById(field_prefix + ".placeholder");
          if (placeholder && placeholder.repeat) {
            GvaScript.Repeat.add(placeholder, i + 1 - placeholder.repeat.count);
            elem = document.getElementById(new_prefix);
          }
        }
